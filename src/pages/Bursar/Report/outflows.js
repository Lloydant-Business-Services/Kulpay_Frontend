...
import { nairaFormat, currencyFormat, nairaFormatAlt } from "../../../utils/helpers";
...
class Outflows extends Component {
    ...
    megaLoader = () => {
        if (this.state.refinementValue == "programme") {
            ...
            this.loadAllCollectionBySession(this.state.vendorId)
        }
    ...
    megaLoader2 = () => {
        if (this.state.refinementValue2 == "programme") {
            this.loadBankOutflowByProgramme()
        }
        else if (this.state.refinementValue2 == "department") {
            this.loadBankOutflowByDepartment()
        }
        else if (this.state.sessionSelect) {
            this.loadBankOutflowBySession()
        }
        else if (this.state.refinementValue == "level") {
    ...
    loadBankOutflowByProgramme = () => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getBankOutflowByProgramme(this.state.payLoad?.institutionId, parseInt(this.state.bankSelect), parseInt(this.state.sessionSelect), parseInt(this.state.programmeSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
                ...
                var mappedData = res.data?.reportList.map((x, i) => {
                    return {
                        ...
                        programme: x.programme.toUpperCase(),
                        amountAlt: x.recievedAmount != null ? nairaFormatAlt(x.recievedAmount, 2) : null,
                ...
                this.processAmount(res.data?.totalAmount)
                setTimeout(() => {
    ...
    loadBankOutflowByDepartment = () => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getBankOutflowByDepartment(this.state.payLoad?.institutionId, parseInt(this.state.bankSelect), parseInt(this.state.sessionSelect), parseInt(this.state.departmentSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
    ...
    loadBankOutflowBySession = (data) => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getBankOutflowBySession(this.state.payLoad?.institutionId, parseInt(this.state.bankSelect), parseInt(this.state.sessionSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
    ...
    getAllVedors = () => {
        Endpoint.getAllVendorByInstitution(this.state.payLoad?.institutionId)
            .then((res) => {
                console.log(res, "vend")
                var filterInstAcct = res.data?.filter((x) => { return !x.isInstitutionAccount })
                console.log(filterInstAcct, "filterInstAcct")
                this.setState({ vendorList: filterInstAcct })
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    loadAllCollectionByProgramme = (data) => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getAllCollectionOutflowByProgramme(parseInt(this.state.vendorId), parseInt(this.state.sessionSelect), parseInt(this.state.programmeSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
    ...
    loadAllCollectionByDepartment = (data) => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getAllCollectionOutflowByDepartment(parseInt(this.state.vendorId), parseInt(this.state.sessionSelect),  parseInt(this.state.departmentSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
    ...
    loadAllCollectionBySession = (data) => {
        // getAllCollectionByPrimaryVendor
        Endpoint.getVendorsOutflowDetailsBySession(parseInt(data), parseInt(this.state.sessionSelect), this.state.dateFrom, this.state.dateTo)
            .then((res) => {
                //alert("1")
                console.log(res.data)
                this.setState({ loadSpinner: false });
    ...
    resolveTotalInflowSum = (data) => {
        ...
        Endpoint.getOutflowSum(this.state.payLoad?.institutionId, collectionId)
            .then((res) => {
    ...
    handleInput = (event) => {
        ...
        if (!value) {
            this.setState({
                refinement: false,
                programmeSelect: 'refine',
                departmentSelect: 'refine',
                // sessionSelect: 'refine',
                levelSelect: 'refine',
    ...
    fetchInflowBanks = () => {
        Endpoint.getOutflowBanks(this.state.payLoad?.institutionId)
            .then((res) => {
            ...
            <>
                ...
                <Fade>
                    <div className="container-fluid py-5 mt-3">
                            ...
                            <div className="row">
                                <div className="col-12 col-sm-12 col-xl-6 mt-2 mt-xl-0">
                                    <h1 className="manrope-text" style={!isMobile ? { fontSize: "33px", color: "#2E2C34" } : { fontSize: "24px", color: "#2E2C34" }}>
                                        ...
                                        Outflow Report <span style={{ fontSize: "18px" }}>(Collection)</span>
                                    </h1>
                                    <Alert style={{ width: '400px' }} showIcon icon={<i className="fa fa-info-circle" />} type="success" message={<p className="manrope-text-light" style={{ fontSize: "10px" }}>
                                        The outflow reports simply presents/show funds that went to beneficiaries. This can be queried majorly using the collection and bank categories
                                    </p>} />
                        ...
                        <Fade style={{ marginTop: "-140px" }}>
                            <div className="row" style={{ textAlign: 'right' }}>
                              
                            </div>
                            <br />
                            <p className="manrope-text" style={{ fontSize: "14px" }}>Select outflow category</p>
                            <div>
                                ...
                                <Radio.Group style={{ marginTop: '-10px'}}  value={this.state.inflowSelectValue} defaultValue="collection" className="manrope-text" onChange={(e) => this.setInflowType(e.target.value)}>
                                    <Radio.Button style={{ fontSize: "11px" }} value="collection">Beneficiary</Radio.Button>
    ...
    <DatePicker onChange={this.onChangeDateTo} />
                            ...
                            <Space>
                                ...
                                {this.state.inflowSelectValue == "collection" ? <Space>
                                    <Select defaultValue="all" style={{ width: 288, fontSize: 12 }} onChange={this.handleVendor}>
                                        <OptGroup label="Collection">
                                            <Option value="">Select Beneficiary</Option>
                                            <Option value="all">All Beneficiaries</Option>
                                            {this.state.vendorList && this.state.vendorList.map(x => {
                                                return (
                                                    <Option value={x.id}>{x.name}
                                                     - {x.bankDetailList[0]?.accountNumber}
                                                     </Option>
                                                )
                                            })}
                                ...
                                {this.state.inflowSelectValue == "bank" ?
                                    ...
                                    <button id="send_btn"
                                        className="btn btn-primary" onClick={this.megaLoader2}>Load
                                    </button>
                                    :
                                    <button id="send_btn"
                                        className="btn btn-primary" onClick={this.megaLoader}>Load
                                    </button>}
                            </Space>
                            <br />
