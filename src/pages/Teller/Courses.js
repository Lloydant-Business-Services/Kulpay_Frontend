...
class HODCourses extends Component {
	...
	createCourse = (e) => {
		...
		Endpoint.createCourse(courseProps)
			.then((res) => {
				...
				this.setState({error: false, success: true, newCourseLoading: false, newCourse: false, newCourseTitle: ''});
	...
	assignCourse = (e) => {
		e.preventDefault();
		if (!this.state.assignCourseId || !this.state.assignInstructorId) {
				...
				<div className="container-fluid py-5">
					<div className="d-flex flex-wrap justify-content-between">
						<div>
							...
							<p className="mb-0">
								<Unicons.UilInfoCircle size="20" className="mr-2"/>
								After creating a course, assign it to a departmental instructor to see it in the list.
							</p>
						</div>
						
						<div>
							<button className="btn btn-outline-primary" onClick={this.openAssignCourse}>
								<Unicons.UilUserCheck size="20"/> Allocate Course
							</button>
							
							<button className="btn btn-primary" onClick={this.openNewCourse}>
									...
									<div className="mt-3">
										{this.state.assignCourseFormIncomplete ?
											<div
												className="bg-danger border-rad-full text-center p-2 mb-3 custom-form-alert">
												<p className="small text-white mb-0">
													<Unicons.UilExclamationCircle size="20"/> Please select instructor and course.
												</p>
											</div>
											: null
										}
